...
export const profile = {
	...
	title: 'Web Engineer',
	lead: [
		'アジャイル(XP)を軸に、要件定義から運用まで一貫してWebアプリケーション開発に携わってきました。',
		'MVPを早く届けてフィードバックを得ながら、柔軟に方針を変えて開発していく動き方が得意です。',
		'マイクロサービスで言語やフレームワークが分かれた現場での経験が長いので、キャッチアップしながら開発するのも得意です',
	],
};
...
export const projects: Project[] = [
	{
		...
		challenge:
			'社内に詳しい人が残っていないレガシーシステム改修で、コード品質も低くインシデントリスクが高かった。何をもって改修が十分なのかも自信を持ちにくく、チームメンバー全員歴が浅く組織の文化や思想も揃いきらない状態だった。当初はTDDで動作を担保しながら進めたが、テスト対象外の深い動線でインシデントを発生させてしまった。以降はペアプロで作業の強度を上げ、誰へのどんな価値かを明確にしてから着手することで完了条件とリリース後の監視を意識する進め方に変えた。地雷は踏む前提でデグレをすぐ自分たちで気づけるようにし、メンバーの考え方を補い合いながら、それ以降インシデントなくレガシーをリファクタリングしつつ進められている。',
	},
	{
